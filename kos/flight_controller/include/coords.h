...
constexpr int64_t A = 6378137000; 	//большая полуось Земли в см
...
struct Vector3D
{
    ...
    {
        double latitude = latit/1e7;
        double longitude = longit/1e7;

        double nu = A /(std::sqrt(1 - std::pow(E*sin(latitude),2)));	// радиус кривизны в главном вертикале
